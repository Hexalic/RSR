...
GameServer.addNewPlayer = function(socket,data){
    ...
    if(!data.pass || data.pass.length == 0) return;
    var player = new Player(data.name, data.pass); 
    var document = player.dbTrim();
    var uniqueName;
    GameServer.server.db.collection('players').findOne({name: data.name},function(err,doc){
        if(err) throw err;
        if(!doc) {
    ...
    if(data.name == uniqueName) return;
