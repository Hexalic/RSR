...
Client.getInitRequest = function(){ // Returns the data object to send to request the initialization data
    ...
    if(Client.isNewPlayer()) return {new:true,name:Client.getName(),pass:Client.getPass(),clientTime:Date.now()};
...
Client.isNewPlayer = function(){
    ...
    var pass = Client.getPass();
    var armor = Client.getArmor();
    var weapon = Client.getWeapon();
    return !(id !== undefined && name && pass && armor && weapon);
};
...
Client.setPass = function(pass){
    localStorage.setItem('pass',pass);
};

Client.getPass = function(){
...
Client.createPlayer = function(){
    if(Client.isNewPlayer()) {
        Client.socket.emit('createacc',{new:true,name:Client.getName(),pass:Client.getPass(),clientTime:Date.now()});
        console.log('Create player - client' , Client.getName());
    }
}
